...
def process_into_dimensional_model():
    """Procesa los datos y guarda el modelo dimensional."""
    try:
        ...
        logging.info("✅ Modelo dimensional procesado y guardado.")
    except Exception as e:
        logging.error(f"❌ Error en `process_into_dimensional_model()`: {e}")
...
# **Tarea: Stream de Kafka**
def stream_hechos_to_kafka(topic: str = 'road_accidents', sleep_time: float = 0.5):
    """
    Transmision de hechos_accidentes.csv al Kafka topic.
    """
    file_path = os.path.join(TRANSFORMED_DIR, "hechos_accidentes.csv")
    
    if not os.path.exists(file_path):
        logging.error(f"File not found: {file_path}")
        raise FileNotFoundError(f"{file_path} does not exist.")

    df_hechos = pd.read_csv(file_path)
    logging.info(f"Read {len(df_hechos)} rows from {file_path}")

    producer = kafka_producer()
    send_df_to_kafka(df_hechos, topic=topic, producer=producer, sleep_time=sleep_time)
...
dimensional_task = PythonOperator(
    task_id='process_into_dimensional_model',
    python_callable=process_into_dimensional_model,
    dag=dag,
)
...
kafka_task = PythonOperator(
    ...
    python_callable=stream_hechos_to_kafka,
    dag=dag,
)
...
[transform_postgres_task, transform_api_task] >> merge_final_task >> gx_validation_task >> dimensional_task >> [load_task, kafka_task]
